      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            おかえりなさい、{profile?.display_name || user?.email?.split('@')[0]}さん
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">参加した試合</p>
                <p className="text-2xl font-bold text-gray-900">{dashboardData.stats.totalGames}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{dashboardData.stats.totalTeams}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{dashboardData.upcomingGames.length}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            {dashboardData.teams.length === 0 ? (
              <div className="text-center py-8">
                ...
                <h4 className="text-lg font-medium text-gray-900 mt-2">チームがありません</h4>
                <p className="text-gray-500 text-sm mt-1">新しいチームを作成しましょう</p>
                <Link href="/teams" className="mt-3 inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">
                  チームを作成
                </Link>
              </div>
            ) : (
                    ...
                    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {team.member_count}人 • {
                            team.user_role === 'owner' ? 'オーナー' : 
                            team.user_role === 'manager' ? 'マネージャー' : 'プレイヤー'
                          }
                        </p>
                      </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link href="/games?filter=upcoming" className="text-blue-600 hover:text-blue-700 text-sm font-medium">
                すべて見る →
              </Link>
            </div>
            {dashboardData.upcomingGames.length === 0 ? (
              <div className="text-center py-8">
                ...
                <h4 className="text-lg font-medium text-gray-900 mt-2">予定された試合がありません</h4>
                <p className="text-gray-500 text-sm mt-1">新しい試合を作成しましょう</p>
                <Link href="/create-game" className="mt-3 inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">
                  試合を作成
                </Link>
              </div>
            ) : (
              <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                          ...
                          {game.status === 'scheduled' ? '予定' : 
                           game.status === 'in_progress' ? '進行中' : game.status}
                        </span>
                      </div>
                      <div className="text-sm text-gray-600">
                        <p>{new Date(game.date).toLocaleDateString('ja-JP')} {game.start_time || ''}</p>
                        <p>{game.venue}</p>
                        {game.participants.length >= 2 && (
                          <p className="mt-1">
                            {game.participants[0].participant_name} vs {game.participants[1].participant_name}
                          </p>
                        )}
                      </div>
                    </div>
                ...
                {dashboardData.upcomingGames.length > 3 && (
                  <div className="text-center pt-2">
                    <Link href="/games?filter=upcoming" className="text-blue-600 hover:text-blue-700 text-sm">
        ...
        {dashboardData.recentGames.length > 0 && (
          <div className="mt-8 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-900">最近の試合結果</h3>
              <Link href="/games?filter=completed" className="text-blue-600 hover:text-blue-700 text-sm font-medium">
                すべて見る →
              </Link>
            </div>
                      ...
                      <div className="space-y-2">
                        {game.participants.map(participant => (
                          <div key={participant.id} className="flex justify-between items-center">
                            <span className={`text-sm ${participant.is_home ? 'text-blue-600' : 'text-red-600'}`}>
